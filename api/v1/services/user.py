...
class UserService(Service):
    ...
    def get_user_by_id(self, db: Session, id: str) -> User:
        """Fetches a user by their id"""
    ...
    def updatepassword(self, db: Session, current_user: User, schema, id=None):
        """Function to update a User"""
        """# Get user from access token if provided, otherwise fetch user by id
        if db.query(User).filter(User.id == schema.id).first():
            raise HTTPException(
                status_code=400,
                detail="User with this email or username already exists",
            )"""
    ...
    def refresh_access_token(self, current_refresh_token: str):
        ...
        token = self.verify_refresh_token(current_refresh_token)
    ...
    def fetch_user_refresh_token(self, request, db: Session):
